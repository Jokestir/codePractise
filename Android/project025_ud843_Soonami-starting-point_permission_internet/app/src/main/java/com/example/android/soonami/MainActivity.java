    ...
    private class TsunamiAsyncTask extends AsyncTask<URL, Void, Event> {
        ...
        private String makeHttpRequest(URL url) throws IOException {
            ...
            try {
                ...
                //if the request was successful (response code 200), then read input from input stream and parse the response
                if(urlConnection.getResponseCode() == 200) {
                    inputStream = urlConnection.getInputStream();
                    jsonResponse = readFromStream(inputStream);
                }
                else{
                    Log.e(LOG_TAG,"The http response code is not 200. The response code is: " + urlConnection.getResponseCode());
                }
            } catch (IOException e) {
