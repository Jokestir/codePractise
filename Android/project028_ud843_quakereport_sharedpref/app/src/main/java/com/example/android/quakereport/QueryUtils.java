...
public final class QueryUtils {
    ...
    private static String convertUnixtimeToReadableString(long unixtime){
        Date d = new Date(Long.valueOf(unixtime));
...
*/
     ...
     */
    public static List<EarthquakeEvent> extractEarthquakes(String urlString) {
        // Create an empty ArrayList that we can start adding earthquakes to
        Log.e(LOG_TAG,"inside extract earthquakes");
        List<EarthquakeEvent> eventArrayList = new ArrayList <>();
        ...
        eventArrayList = extractFeatureListFromJson(JsonResponse);
        // Return the list of earthquakes
        return eventArrayList;
    }

    private static List<EarthquakeEvent> extractFeatureListFromJson(String jsonResponse) {
        if(jsonResponse == null)
            return  null;

        List<EarthquakeEvent> returnArrayList = new ArrayList <EarthquakeEvent>();
        try {
            JSONObject rootJsonObj = new JSONObject(jsonResponse);
            JSONArray featuresJsonArray = rootJsonObj.getJSONArray("features");

            for(int i =0;i<featuresJsonArray.length();i++){
                JSONObject currentPropertiesJsonObj = featuresJsonArray.getJSONObject(i).getJSONObject("properties");
                String cityLoc = currentPropertiesJsonObj.getString("place");
                String magnitude = currentPropertiesJsonObj.getString("mag");
                long timeinmillis = currentPropertiesJsonObj.getLong("time");
                String url = currentPropertiesJsonObj.getString("url");

                returnArrayList.add(new EarthquakeEvent(cityLoc,Double.valueOf(magnitude),timeinmillis,url));
            }
    ...
    private static String makeHttpConnection(URL url) throws IOException {
        Log.e(LOG_TAG,"inside makehttpconnection");
        ...
        try {
            ...
            if(httpconnection.getResponseCode() == 200){
                ...
                Log.e(TAG,"http responsecode is not 200. response: " + httpconnection.getResponseCode());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
